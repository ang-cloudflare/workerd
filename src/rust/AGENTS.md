...
- **JSG resources**: `#[jsg_resource]` on struct + impl block; `#[jsg_method]` auto-converts `snake_case` → `camelCase`; resources integrate with GC via the `GarbageCollected` trait (auto-derived for `Ref<T>`, `WeakRef<T>`, `Option<Ref<T>>`, and `Nullable<Ref<T>>` fields)
