 ...
 public:
  ...
  ExecutionContext(jsg::Lock& js,
    ...
    if (FeatureFlags::get(js).getEnableVersionApi()) {
      version = kj::mv(versionInfo).map([&js](auto v) { return jsg::JsRef{js, v.toJs(js)}; });
    }
  }
  ...
  jsg::JsValue getVersion(jsg::Lock& js) {
    ...
    //   `ctx` object instantiation" are able to trigger this assertion. For now we want this
    //   failure to only be realised once `ctx.version` is accessed rather than when calling the
    //   worker's entrypoint.
